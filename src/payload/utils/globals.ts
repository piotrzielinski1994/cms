...
const getGlobal = async (slug: keyof Config['globals'], locale: TypedLocale = 'en', depth = 1) => {
  const payload = await getPayload({ config: configPromise });
  return payload.findGlobal({ slug, depth, locale });
};

const getCachedGlobal = (slug: keyof Config['globals'], locale: TypedLocale, depth = 1) => {
