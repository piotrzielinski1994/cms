...
describe('Pages collection', () => {
  // AC-010 / TC-016: the content block config is registered in the sections
  // field so editors can add a Content block and its blockType joins the union.
  it('should register the "content" block in the sections blocks array', () => {
    const slugs = collectBlockSlugs(Pages.fields);
    expect(slugs).toContain('content');
  });
});
